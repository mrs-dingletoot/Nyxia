...
export async function messageCreate(client, message) {
 ...
 if (message.mentions.users.has(client.user.id) && (!message.reference || !message.reference.messageId)) {
  const embed = new EmbedBuilder()
   .setTitle("👋 Hello!")
   .setDescription(
    `Hello ${message.author}! I'm ${client.user.username}, a multi-purpose Discord bot created for memes, image editing, giveaways, moderation, anime, utility, statistics and so much more!:fire:

   > **You can find the list of all my commands by typing \`/help\`** or by visiting [my dashboard](${client.config.url}/commands)

   *Invite me to your server by clicking [here](${client.config.url}/api/invite)!*
   `
   )
 ...
 if (
    ...
    message.channel.type !== ChannelType.GuildMedia &&
 ...
 if (!messages) {
  await prismaClient.guildMessage.create({
   data: {
    guild: {
     connectOrCreate: {
      where: {ID: new ObjectId().toString(), guildId: message.guild.id },
